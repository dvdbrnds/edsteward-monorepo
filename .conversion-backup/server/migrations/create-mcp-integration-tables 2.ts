/**
 * Database migration script to create MCP integration tables
 * 
 * This script should be run once to add the required tables for MCP client integration:
 * - regulation_versions
 * - validation_status
 * - sync_control
 * - notification_queue
...
async function createMCPIntegrationTables() {
  console.log('Starting creation of MCP integration tables...');
  syslog.log(LogFacility.LOCAL0, LogLevel.INFO, "Starting MCP integration tables migration");
  
  try {
    // Check if tables already exist to avoid duplicate migration
    const tablesExist = await checkIfTablesExist();
    
    if (tablesExist) {
      console.log('MCP integration tables already exist. Skipping migration.');
      syslog.log(LogFacility.LOCAL0, LogLevel.INFO, "MCP integration tables already exist, migration skipped");
      return;
    }
    ...
    syslog.log(LogFacility.LOCAL0, LogLevel.INFO, "MCP integration tables migration completed successfully");
  } catch (error) {
    ...
    // Close pool is not necessary here since it's used by the application
  }
}
...
export { createMCPIntegrationTables };