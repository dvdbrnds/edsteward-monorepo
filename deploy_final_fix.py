...
def deploy_final_fix():
    """Deploy the final fix with the clean build"""
    try:
        ...
        print(f"🚀 Deploying FINAL fix image: {fix_image}")
        ...
        for i in range(20):  # 10 minutes max
            ...
            if primary_deployment:
                ...
                if running_count == desired_count and new_task_arn in current_task_def:
                    ...
                    time.sleep(15)  # Wait a bit longer for the service to be ready
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if len(data) > 0:
                                first_reg = data[0]
                                print(f"📊 First regulation has {len(first_reg.keys())} fields")
                                
                                # Check for the critical fields
                                critical_fields = ['requirements', 'regulationUrl', 'submissionGuidelines', 'regulationText', 'complianceNotes', 'sections']
                                present_fields = [field for field in critical_fields if field in first_reg]
                                missing_fields = [field for field in critical_fields if field not in first_reg]
                                
                                print(f"✅ Present fields: {present_fields}")
                                print(f"❌ Missing fields: {missing_fields}")
                                
                                if len(present_fields) >= 3:  # At least half the critical fields
                                    print("🎉 SUCCESS! The fix is working - critical fields are now present!")
                                    return True
                                else:
                                    print("⚠️ Fix deployed but most critical fields still missing")
                                    return False
                            else:
                                print("⚠️ Fix deployed but no regulations returned")
                                return False
                        else:
