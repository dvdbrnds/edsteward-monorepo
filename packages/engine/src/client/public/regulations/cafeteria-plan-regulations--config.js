/**
 * Auto-generated config for: Cafeteria Plan Regulations
 * Migrated from v1 inline console to v2 shared template.
 *
 * To add bespoke tasks, deadlines, and penalties, populate
 * window.REGULATION_CONFIG.tasks / .deadlines / .penalties below.
 */

window.REGULATION_SLUG = 'cafeteria-plan-regulations';
window.REG_KEY = 'REG-230';
window.JURISDICTION_SOURCE = 'federal';
window.STATE_CODE = '';
window.ENFORCING_AGENCY = '';
window.REGULATION_NAME = 'Cafeteria Plan Regulations';

window.REGULATION_CONFIG = {
  id: 'cafeteria-plan-regulations',
  name: 'Cafeteria Plan Regulations',
  jurisdiction: 'federal',
  stateCode: '',
  enforcingAgency: '',
  tasks: [],
  deadlines: [],
  penalties: [],
};
