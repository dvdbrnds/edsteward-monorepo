/**
 * Auto-generated config for: Lobbying Disclosure Act
 * Migrated from v1 inline console to v2 shared template.
 *
 * To add bespoke tasks, deadlines, and penalties, populate
 * window.REGULATION_CONFIG.tasks / .deadlines / .penalties below.
 */

window.REGULATION_SLUG = 'lobbying-disclosure-act';
window.REG_KEY = 'REG-185';
window.JURISDICTION_SOURCE = 'federal';
window.STATE_CODE = '';
window.ENFORCING_AGENCY = '';
window.REGULATION_NAME = 'Lobbying Disclosure Act';

window.REGULATION_CONFIG = {
  id: 'lobbying-disclosure-act',
  name: 'Lobbying Disclosure Act',
  jurisdiction: 'federal',
  stateCode: '',
  enforcingAgency: '',
  tasks: [],
  deadlines: [],
  penalties: [],
};
