...
EdSteward has updated all regulation master key field numbers to fix the MCP
...
- **Before**: MCP Engine sent regulation ID 269 → EdSteward mapped to 4727 →
  Database error (4727 didn't exist)
- **After**: MCP Engine sends regulation ID 269 → EdSteward uses 269 directly →
  Success! ✅
...
Delete or disable any function that converts regulation IDs. The mapping is no
longer needed.
...
_Note: Complete mapping available in CSV files if needed_
...
**Problem**: Only regulation ID 55 worked because EdSteward database had IDs
4459-4852, but MCP Engine was mapping to non-existent IDs.

**Solution**: EdSteward updated all regulation master key fields to sequential
numbers 1-354.
...
If you need the complete regulation mapping in CSV format or have questions,
contact the EdSteward team.
