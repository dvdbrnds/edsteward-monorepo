...
async function testDatabaseOperations() {
  ...
  try {
    ...
    log(`Note created: ${JSON.stringify(createdNote)}`);
    ...
    if (createdNote && createdNote.id) {
      ...
      log(`Note retrieved: ${JSON.stringify(retrievedNote)}`);
      ...
      log(`Note updated: ${JSON.stringify(updatedNote)}`);
      ...
      log(`Note after deletion: ${JSON.stringify(noteAfterDeletion)}`);
